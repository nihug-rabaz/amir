...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/users')
      .then((r) => r.json())
      .then((j) => { if (active) setUsers((j.users || []).filter((u: User) => u.active)); })
      .catch(() => { if (active) toast.danger('שגיאת חיבור', 'לא ניתן לטעון משתמשים'); })
  ...
  function pick(u: User) {
    setFade(true);
    setTimeout(() => {
      toast.success(`ברוך הבא, ${u.name}`, ROLE_LABELS[u.role]);
      signIn(u);
    }, 220);
  }

  function manualLogin() {
    if (!personalId.trim()) return toast.danger('שגיאה', 'יש להזין מספר אישי / ת״ז');
    const u = users.find((x) => x.personalId === personalId.trim());
    if (!u) return toast.danger('משתמש לא נמצא', 'בחר מהרשימה למטה');
    pick(u);
  }
